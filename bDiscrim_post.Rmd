...
\footnotesize
```{r}
cbind(hilo, hilo.pred$x) %>% arrange(desc(LD1)) -> d
d
```
\normalsize
...
```{r}
d.1 <- lm(LD1 ~ yield + weight, data = d)
summary(d.1)
```
...
* Here, IQRs both 2.25, *identical*, so 1-unit change in each variable
means same thing.
...
\footnotesize
```{r}
d <- cbind(hilo, predicted = hilo.pred$class)
d
with(d, table(obs = fertilizer, pred = predicted))
...
```{r}
ggplot(mm, aes(x=smk, y = LD1)) + geom_point()
```
...
`geom_text_repel` is from package `ggrepel`.

```{r}
g <- ggplot(mm, aes(x = LD1, y = LD2, colour = combo, 
                    label = combo)) + geom_point() +
  geom_text_repel() + guides(colour = FALSE)
