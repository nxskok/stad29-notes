...
\begin{frame}[fragile]{`menheal` by itself}
...
\begin{frame}[fragile]{Residual plot (from `timedrs` on all)}
...
\begin{frame}[fragile]{Just `right`}
{\small
```{r }
...
\begin{frame}[fragile]{Augmenting `punting.2`}
```{r size="footnotesize"}
...
\begin{frame}[fragile]{Residuals against `left`}
...
\begin{frame}[fragile]{Regression with `left`-squared}
```{r size="scriptsize"}
summary(punting.3)
```
