...
```{r}
rat2 %>% pivot_longer(-dose, names_to = "status", values_to = "freq") %>% 
  uncount(freq) -> rat2x
rat2x
```
...
```{r}
rat2x.1 <- glm(factor(status)~dose, data = rat2x, family = "binomial")
summary(rat2x.1)
```
...
```{r }
freqs %>%
  pivot_longer(None:Severe, names_to = "Severity", values_to = "Freq") %>%
  group_by(Exposure) %>%
...
```{r size="small"}
miners.pred %>%
  pivot_longer(-Exposure, names_to = "Severity", values_to = "probability") %>%
